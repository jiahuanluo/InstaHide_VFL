...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, cfg=None):
        ...
        self.conv2 = ConvBlock(planes, planes, kernel_size=3,
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                ConvBlock(in_planes, self.expansion * planes,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv1 = ConvBlock(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = ConvBlock(planes, planes, kernel_size=3,
                               stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = ConvBlock(planes, self.expansion *
...
class ResNet_Top(nn.Module):
    def __init__(self, block, num_blocks, model_config, num_classes=10):
        ...
        self.layer0 = nn.Sequential(
            ConvBlock(3, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),

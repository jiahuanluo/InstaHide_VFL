...
import models
from utils import progress_bar, chunks
...
def train(bottom_model, top_model, bottom_optimizer, top_optimizer, inputs_all, mix_targets_all, lams, epoch):
    print('\nEpoch: %d' % epoch)
    bottom_model.train()
    top_model.train()
    ...
    for batch_idx in range(len(bl)):
        ...
        output_tensor_bottom_model_a = bottom_model(inputs)
        ...
        outputs = top_model(input_tensor_top_model_a)
...
def test(bottom_model, top_model, bottom_optimizer, top_optimizer, testloader, epoch, start_epoch):
    global best_acc
    bottom_model.eval()
    top_model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            output_tensor_bottom_model_a = bottom_model(inputs)
            outputs = top_model(output_tensor_bottom_model_a)
    ...
    if epoch == start_epoch + args.epoch - 1 or acc > best_acc:
        save_checkpoint(bottom_model, top_model, acc, epoch)
    if acc > best_acc:
        best_acc = acc
    return (test_loss / batch_idx, 100. * correct_1 / total)


def save_checkpoint(bottom_model, top_model, acc, epoch):
    """ Save checkpoints. """
    # print('Saving..')
    # state = {
    #     'net': net,
    ...
    # }
    save_path = f'checkpoint_{args.model}_{args.data}_{args.mode}_{args.klam}'
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    ckptname = os.path.join(save_path, f'passive_model.pt')
    torch.save(bottom_model, ckptname)
    ckptname = os.path.join(save_path, f'top_model.pt')
    torch.save(top_model, ckptname)
...
def main():
    ...
    if args.resume:
        ...
        bottom_model = models.ResNet18_Bottom(None)
        top_model = models.ResNet18_Top(None)
    save_path = f'checkpoint_{args.model}_{args.data}_{args.mode}_{args.klam}'
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    logname = f'{save_path}/log_{args.model}_{args.data}_{args.mode}_{args.klam}_{args.name}_{args.seed}.csv'
    ...
    bottom_optimizer = optim.SGD(bottom_model.parameters(),
                                 lr=args.lr,
                                 momentum=0.99,
                                 weight_decay=args.decay)

    top_optimizer = optim.SGD(top_model.parameters(),
                              lr=args.lr,
                              momentum=0.99,
                              weight_decay=args.decay)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_loss, _ = train(
            bottom_model, top_model, bottom_optimizer, top_optimizer, mix_inputs_all, mix_targets_all, lams, epoch)
        test_loss, test_acc1, = test(
            bottom_model, top_model, bottom_optimizer, top_optimizer, testloader, epoch, start_epoch)
        adjust_learning_rate(bottom_optimizer, epoch)
        adjust_learning_rate(top_optimizer, epoch)
